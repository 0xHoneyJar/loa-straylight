...
//
// It ALSO owns the workflow boundary's READ half (rev. 3 §11, round-10
// J2/J3): the closed `straylight.read-plan.v1` schema and read-kind
// registry, the `straylight.fetch-slot-claim.v1` contract, and the
// gather read-ledger contract — see the "Read contracts" section below.
...
import { REPOSITORY_ALLOWLIST } from "./write-plan.mjs";
...
// =============================================================================
// Read contracts — the workflow boundary's READ half (rev. 3 §11,
// round-10 J2/J3; folded from the retired lib/read-plan.mjs)
// =============================================================================
//
// Every DERIVED fetch — one whose target was computed from evidence (a
// lane's recorded PR number, a PR's head SHA, a collection's issue
// slots) — is expressed as a closed `straylight.read-plan.v1` document
// authored by a planner/collector binary and executed by the single
// shared read executor (bin/execute-read-plan.mjs). Bash never routes on
// evidence: it fetches FIXED urls to files, switches on validated exit
// codes, and invokes the checked-in Node entry points.
//
// This section owns:
...
//     collection contract above); a PR/check/status fetch failure is a
//     DURABLE ledger fact ({fetched:false}), never filename absence;
...
//   ledger rows use the watchdog collection format (parseLedger above)
//   so the seal/verify stages consume them unchanged.
...
const READ_PLAN_KEYS = Object.freeze(["schema", "plan_id", "nonce", "repository", "collection_id", "reads"]);
...
export function validateReadPlan(plan, { repository, nonce } = {}) {
  ...
  for (const key of Object.keys(plan)) {
    if (!READ_PLAN_KEYS.includes(key)) errors.push(err("unknown-field", `plan.${key}`));
  }
...
const READ_ROW_KEYS = Object.freeze(["nonce", "gather", "slot", "pr_number", "sha", "fetched", "path", "sha256"]);
...
export function parseReadLedger(text, { nonce } = {}) {
  ...
  for (const line of text.split("\n")) {
    ...
    for (const key of Object.keys(row)) {
      if (!READ_ROW_KEYS.includes(key)) return bad("read-ledger-row-unknown-field", key);
    }
