...
export function devSign(params) {
    const payload_hash = sha256(params.payload);
    const sig = devSignatureFor(params.key_ref, payload_hash);
    return {
        signature_id: contentId({ payload_hash, signer_id: params.signer_id, signed_at: params.signed_at }, { prefix: 'sig' }),
...
export function devSignatureFor(key_ref, payload_hash) {
...
export function verifyDevSignature(envelope, payload, expected_key_ref) {
...
export function verifyEnvelopeSelfConsistency(envelope) {
    if (envelope.signature_type !== 'dev_signature') {
        return { valid: false, reason: `unsupported_signature_type:${envelope.signature_type}` };
    }
