...
export function policyForAdmitAssertion(input) {
    return safeRun(input.now, () => admitInner(input));
}
function admitInner(input) {
    if (!input.class_validation.valid) {
        return deny(['class_validation_failed', ...input.class_validation.errors.map((e) => `class:${e.code}:${e.path}`)], { allowed: false, reason: 'class_validation_failed' }, input.now);
    }
...
export function policyForTransition(input) {
    return safeRun(input.now, () => transitionInner(input));
}
function transitionInner(input) {
...
export function policyForRecallRequest(input) {
    return safeRun(input.now, () => recallInner(input));
}
function recallInner(input) {
    ...
    const isPublicFrame = input.request.environment_frame === 'public_discord' ||
        input.request.environment_frame === 'public_telegram';
    const publicHighRiskNeedsReview = isPublicFrame && (input.request.risk_profile === 'high' || input.request.risk_profile === 'critical');
    return projectCompetence({
        competence,
        now: input.now,
...
export function dispositionFor(assertion, request) {
    ...
    if ((request.risk_profile === 'high' || request.risk_profile === 'critical') &&
...
function privacyDispositionForFrame(privacy, frame) {
    const isPublic = frame === 'public_discord' || frame === 'public_telegram';
    const isAudit = frame === 'audit_review';
    if (privacy === 'sealed') {
        if (isAudit)
            return { kind: 'redact', reason: 'privacy_sealed_audit_redacted' };
        return { kind: 'exclude', reason: 'privacy_sealed' };
    }
    if (privacy === 'actor_private') {
        if (isPublic)
            return { kind: 'exclude', reason: 'privacy_actor_private_in_public_frame' };
        if (frame === 'private_chat')
            return { kind: 'exclude', reason: 'privacy_actor_private_in_chat_frame' };
    }
...
function hasNonModelOutputProvenance(a) {
    return a.provenance.some((p) => p.source_type !== 'model_output');
}
function projectCompetence(input) {
...
function needsReviewForModelOutput(c) {
...
function verifyAllSignatures(envs) {
    for (const e of envs) {
        const r = verifyEnvelopeSelfConsistency(e);
        if (!r.valid)
            return { ok: false, reason: r.reason ?? 'signature_invalid' };
    }
    return { ok: true };
}
function deny(reasons, competence, now, decision = 'deny') {
    return {
        decision,
        policy_id: DEFAULT_POLICY_ID,
        policy_version: DEFAULT_POLICY_VERSION,
