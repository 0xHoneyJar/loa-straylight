...
import { checkSameTenant } from './tenancy.js';
export function handleRecallIntake(store, req, deps) {
    ...
    const targetCheck = checkSameTenant(req.caller.tenant_id, req.request.actor_id, deps.tenantResolver);
    if (!callerCheck.ok || !targetCheck.ok) {
        const unresolved = callerCheck.reason === 'tenant_unresolved' || targetCheck.reason === 'tenant_unresolved';
        const reason = unresolved ? 'tenant_resolution_failed' : 'cross_tenant_recall_refused';
        const raw_reasons = [
    ...
    if (outcome.policy_decision.decision === 'needs_review') {
        const handle = contentId({ recall_request_id: req.request.recall_request_id, now: deps.now }, { prefix: 'rqueue' });
        return {
    ...
    const reason = classifyWedgeDeny(outcome.policy_decision.reasons, outcome.class_validation.valid, outcome.policy_decision.signer_competence_result.reason);
    const entry = deps.intakeLog.append({
        caller_tenant: req.caller.tenant_id,
        caller_actor_id: req.caller.actor_id,
        target_actor_id: req.request.actor_id,
...
function classifyWedgeDeny(reasons, classValid, competenceReason) {
    if (!classValid)
        return 'class_validation_failed';
    for (const r of reasons) {
        if (r === 'policy_unavailable_for_transition')
            return 'policy_unavailable';
        if (r.startsWith('signature_invalid:'))
            return 'signer_not_competent';
        if (r.startsWith('competence:'))
            return 'signer_not_competent';
    }
    if (competenceReason === 'no_competence_rule_for_transition')
        return 'policy_unavailable';
