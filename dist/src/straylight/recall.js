...
export function executeRecall(store, request, now) {
    ...
    const redactionOf = (reason) => {
        const existing = redacted.find((r) => r.reason === reason);
        if (existing)
            existing.count += 1;
        else
            redacted.push({ reason, count: 1 });
    };
    const bumpExclusion = (reason) => {
    ...
    for (const a of candidates) {
        ...
        switch (disp.kind) {
            case 'include':
                ...
                break;
            case 'mark':
                marked.push(buildRecallItem(a, useInstructionForMark(disp.reason), request.include_provenance ?? false));
                break;
            case 'redact':
    ...
    const excluded_summary = [...excludedCounts.entries()]
...
function buildRecallItem(a, use_instruction, include_provenance) {
    return {
...
function useInstructionForMark(reason) {
    if (reason.startsWith('status_contested'))
        return 'mark_as_contested';
    if (reason.startsWith('status_demoted'))
        return 'use_as_background_only';
    if (reason.startsWith('status_superseded'))
        return 'use_as_background_only';
    return 'do_not_use_for_action';
}
function recordRecallDeny(store, request, now, policy_decision) {
    const audit_event = store.auditLog.append({
...
function redactReceipt(r) {
    const computeHash = (toHash) => sha256({
