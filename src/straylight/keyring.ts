...
export interface CompetenceQuery {
  ...
  // Context that competence rules can match on:
  assertion_class?: AssertionClass;
  target_class?: AssertionClass;
  risk_level?: RiskLevel;
  environment_frame?: EnvironmentFrame;
  now: string; // ISO8601, for valid_from/valid_until + timelock comparisons
}
...
export function isSignerCurrentlyValid(
  keyring: Keyring,
  entry: SignerEntry,
  now: string,
...
export function evaluateCompetence(
  ...
  // Pass 1: collect every signature whose signer is on-keyring, currently
  // valid, role-allowed (and not role-forbidden). The first such signer
  // becomes the `signer_id`/`signer_role` headline; the full set drives
  // quorum/timelock/review checks.
  type Match = { entry: SignerEntry; sig: SignatureEnvelope };
  const matches: Match[] = [];
  ...
  let timelock_elapsed: number | undefined;
  if (matchedRule.timelock_seconds && matchedRule.timelock_seconds > 0) {
    const earliest = matches
      ...
      .reduce<number | undefined>((acc, n) => (acc === undefined || n < acc ? n : acc), undefined);
...
function findCompetenceRule(
  rules: SignerCompetenceRule[],
  query: CompetenceQuery,
): SignerCompetenceRule | undefined {
...
function ruleMatches(rule: SignerCompetenceRule, q: CompetenceQuery): boolean {
  if (rule.transition_type !== q.transition_type) return false;
  if (rule.assertion_class && rule.assertion_class !== q.assertion_class) return false;
  if (rule.target_class && rule.target_class !== q.target_class) return false;
  if (rule.environment_frame && rule.environment_frame !== q.environment_frame) return false;
...
function specificity(r: SignerCompetenceRule): number {
  let n = 0;
  if (r.assertion_class) n += 1;
  if (r.target_class) n += 1;
  if (r.environment_frame) n += 1;
  if (r.risk_level) n += 1;
  return n;
}

// Convenience: list active signer types (used for diagnostics).
export function listActiveSignerRoles(keyring: Keyring, now: string): SignerType[] {
