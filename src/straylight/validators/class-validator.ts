...
const ASSERTION_CLASSES: ReadonlySet<AssertionClass> = new Set([
  'observation',
  'event',
  'claim',
  'assumption',
...
const ASSERTION_STATUSES: ReadonlySet<AssertionStatus> = new Set([
  'proposed',
  'active',
  'contested',
  'demoted',
...
const PROVENANCE_SOURCE_TYPES: ReadonlySet<ProvenanceRef['source_type']> = new Set([
  'operator_input',
  'runtime_observation',
  'tool_result',
  'model_output',
...
const SIGNATURE_TYPES: ReadonlySet<SignatureEnvelope['signature_type']> = new Set([
  'ed25519',
  'secp256k1',
  'hmac',
  'dev_signature',
...
export function validateCandidateAssertion(
...
export function validateRecallRequest(r: RecallRequest): ClassValidationResult {
  const errors: ValidationError[] = [];
...
function validateSignatureEnvelope(
  ...
  for (const f of ['signer_id', 'signer_type', 'signed_payload_hash', 'signature', 'signed_at', 'key_ref'] as const) {
    if (typeof s[f] !== 'string' || (s[f] as string).length === 0) {
      errors.push({
        path: `${path}.${f}`,
        code: 'missing_field',
...
function validateAssertionEnumArray<T>(
  arr: T[] | undefined,
  path: string,
