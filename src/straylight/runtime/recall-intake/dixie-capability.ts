...
const ISSUED_CAPABILITIES: WeakSet<DixieCapability> = new WeakSet();
 ...
 */
export function createDixieCapability(): DixieCapability {
  const key = readKeyFromEnv();
  if (key === undefined) {
    throw new DixieCapabilityError(
      'capability_unavailable: STRAYLIGHT_RUNTIME_DIXIE_KEY is not set',
    );
  }
  const nonce = randomBytes(NONCE_BYTES).toString('hex');
  const proof = hmac(key, nonce);
  const cap: DixieCapability = Object.freeze({ nonce, proof });
 ...
 */
export function verifyDixieCapability(
  candidate: unknown,
): DixieCapabilityVerdict {
  if (
    typeof candidate !== 'object' ||
    candidate === null ||
    !ISSUED_CAPABILITIES.has(candidate as DixieCapability)
  ) {
    return { ok: false, reason: 'capability_unrecognized' };
  }
  const cap = candidate as DixieCapability;
  const key = readKeyFromEnv();
  if (key === undefined) {
