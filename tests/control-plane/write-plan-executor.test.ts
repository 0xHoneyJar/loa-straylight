...
import { makeEvent } from "./_fixtures.js";
...
for (const util of ["cat", "printf", "rm", "sh"]) {
...
// A per-test gh mock: records each launch's argv (one line per call) and
// stdin (gh-stdin-<n>), then runs the caller-supplied extra shell logic
// with $N (call number) available. Records are written BEFORE behavior
// runs so even a failing call is counted as launched.
function makeGhMock(root: string, extra = "") {
...
describe("write-plan validator — closed schema and structural rules", () => {
  it("row 14: best_effort is not expressible on any operation (unknown-field, kind-derived fatality)", () => {
    ...
    const r = validatePlan(
      { schema: "straylight.write-plan.v1", plan_id: `${NONCE}-t`, nonce: NONCE, repository: REPO, operations: [op] },
  ...
  it("row 17/18: terminal barrier — nothing after an issue's state-advancing op; never two of them", () => {
    ...
    const after = validatePlan(
      {
        schema: "straylight.write-plan.v1", plan_id: `${NONCE}-t`, nonce: NONCE, repository: REPO,
  ...
  it("warning gate: exactly one of warning_op_id / warning_proof; the reference must be an earlier same-issue same-lane warning", () => {
    ...
    const valid = validatePlan(
      { schema: "straylight.write-plan.v1", plan_id: `${NONCE}-t`, nonce: NONCE, repository: REPO, operations: [w, removal] },
      { repository: REPO, nonce: NONCE },
    );
    ...
    for (const bad of [
      ...
      const r = validatePlan(
        { schema: "straylight.write-plan.v1", plan_id: `${NONCE}-t`, nonce: NONCE, repository: REPO, operations: [w, bad] },
        { repository: REPO, nonce: NONCE },
      );
      expect(r.ok).toBe(false);
  ...
  it("nonce discipline: plan.nonce must equal --nonce and both must be run-shaped", () => {
    ...
    const mismatch = validatePlan(
      { schema: "straylight.write-plan.v1", plan_id: "99999-2-t", nonce: "99999-2", repository: REPO, operations: [op] },
      { repository: REPO, nonce: NONCE },
    );
    ...
    for (const badNonce of ["abc", "12345", "0-1", "12345-", "-1", "12345-1-1"]) {
      const r = validatePlan(
        { schema: "straylight.write-plan.v1", plan_id: `${badNonce}-t`, nonce: badNonce, repository: REPO, operations: [op] },
...
describe("executor execution phase — fixed argv, retained bytes, exit 4 on any launch/fatal failure", () => {
  ...
  it("PARTIAL EXECUTION (accepted addendum): op 1 succeeds and is recorded; LAUNCHING op 2 fails; exit 4; nothing later runs", () => {
    ...
    // The mock deletes ITSELF after call 1: op-2's spawn fails (ENOENT) —
    // a LAUNCH failure, not an API failure. PATH is ISOLATED to the mock
    // dir + bare utilities so no system gh can absorb the second launch.
  ...
  it("inability to launch gh on the FIRST operation is exit 4, never exit 2 — execution had begun", () => {
    const root = makeRoot();
    const planPath = makePlan(root, [findingOp("op-1", root)]);
    const r = runExecutor(root, planPath, null); // no gh anywhere on PATH
    expect(r.status).toBe(4);
    expect(r.out.phase).toBe("execution");
    expect(r.out.refusal).toBe("gh-launch-failed");
  });
  ...
  it("inability to launch gh for a WARNING-ONLY label operation is still exit 4 — no trustworthy result exists", () => {
    ...
    const r = runExecutor(root, makePlan(root, [op]), null);
    expect(r.status).toBe(4);
    expect(r.out.refusal).toBe("gh-launch-failed");
  });

  it("an empty validated plan executes nothing and exits 0", () => {
    const root = makeRoot();
    const gh = makeGhMock(root);
